...
namespace CsRGBshow
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            int n = C.Count; //目標個數
            int wd = Pw + (Pw + 2) * n + Pw; //虛擬車牌二值化陣列寬度
            int mx = 0, mk = 0; //空白垂直線最大數，最佳偏移量
            byte[,] S0 = new byte[wd, Ph]; //最佳校正之車牌二值化陣列
            for (int sx = -15; sx < 16; sx++) //嘗試偏移量
            {
                byte[,] S = new byte[wd, Ph]; //虛擬車牌陣列
                double z = (double)sx / (Ph - 1); //每一畫素高度的X錯位量
                for (int j = 0; j < Ph; j++)
                {
                    int dx = (int)(z * (Ph - 1 - j)); //X移動量
                    for (int i = 0; i < wd; i++)
                    {
                        int x = i + dx;
            ...
            if (mk != 0) //需要調整傾倒字元，將修正後之車牌二值化影像重作目標擷取
            {
                ...
                for (int k = 0; k < n; k++)
                {
                    ...
                    T[k] = G; //儲存旋轉後的目標物件
                    w[k] = G.width; h[k] = G.height;
                }
                Array.Sort(w); Array.Sort(h);
                int mw = w[n - 2], mh = h[n - 2];
                for (int k = 0; k < n; k++)
                {
        ...
        private LPInfo getLP(ArrayList A)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                int xs = Pw + (Pw + 2) * k; //X偏移量
                for (int i = 0; i < Pw; i++)
                {
                    for (int j = 0; j < Ph; j++)
                    {
                        V[xs + i, j] = ((byte[,])MC[k])[i, j];
                    }
                }
            }
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            int n = C.Count; //目標個數
            int wd = Pw + (Pw + 2) * n + Pw; //虛擬車牌二值化陣列寬度
            byte[,] S = new byte[wd, Ph]; //最佳校正之車牌二值化陣列
            double z = double.Parse(ComboBox1.Text) / (Ph - 1); //每一畫素高度的X錯位量
            for (int j = 0; j < Ph; j++)
            {
                int dx = (int)(z * (Ph - 1 - j)); //X移動量
                for (int i = 0; i < wd; i++)
        ...
        {
            byte[] q = CsLOblique.Properties.Resources.font;
            int n = 0;
            for (int m = 0; m < 2; m++)
            {
                for (int k = 0; k < 36; k++)
